                            ...
                            menuSubItem(text = "Data Table", tabName = "flowTable", icon= icon("fas fa-table")),
                                       sidebarMenu(width = 300,
                                                   ...
                                                   sliderInput(inputId = "uwfgpay1",
                                                               ...
                                                               min=0.00, max = 4.00, value = c(2.00, 4.00)
                                                               ),
...
')
    ...
    output$flowtable <- renderDT({
        ...
        FlowTable <- flowdata() %>% 
            select("Entry"=UWFFall1CIPTitle,"Year2"=UWFFall2CIPTitle,"Year3"=UWFFall3CIPTitle,
                   "Year4"=UWFFall4CIPTitle,"FinalDegree"=DEGREECIPTitle,"GraduationTime"=Gradu_Time,Cohort,GPA_HIGHSCHOOL,UWFGPAY1 ) %>% 
            group_by(Entry,Year2,Year3,Year4,FinalDegree,GraduationTime,Cohort ) %>% 
            dplyr::summarise(  MinHSGPA=min(GPA_HIGHSCHOOL, na.rm=T),
                               ...
                               MeanUWFGPA= round(mean(UWFGPAY1, na.rm=T),2),
                               Counts = n(), .groups= "drop") %>% ungroup()  
        FlowTable$TotalCount <-  rep(sum(FlowTable$Counts))
        FlowTable$FinalStatus_Rate <- paste0(round(FlowTable$Counts/FlowTable$TotalCount,4)*100, "%", sep="") 
